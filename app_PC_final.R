...
"
    ...
    tags$p(HTML("<b>T</b>ext mining and Machine Driven Classification of Open-ended Responses
    <b>(MDCOR)</b> analyses via Latent Dirichlet Allocation & Gibbs Sampling.<br>
    <b>T</b>he platform identifies the optimal number of code allocations based on four metrics.<br>
...
tags$p(a("About MDCOR",target="_blank",href="DDS_about_PHUDCFILY.pdf" )),
    ...
    # tags$li("Select numbers of most common words to trim (0 removes no words)"),10.2.22
	...
	# div(style = "margin-top: -20px"),				#new PHUDCFILY 10.4.22
    ...
    tags$hr(),
    tags$h4("6-7. Code modeling and most representative texts per code"),


    tags$h5("After MDCOR completion you can download the resulting MLTC datasets"),
    #textOutput("text"),
	...
	withSpinner(verbatimTextOutput("GroupTestPHUDCFILY"), hide.ui = FALSE),
...
x1$text <- gsub("\n", "",   x1$text)
x1$text <- gsub("_\235ª", "",   x1$text)
x1$text <- gsub("_\235_¢", "",   x1$text)
x1$text <- gsub("_\235__", "",   x1$text)
x1$text <- gsub("_\235_Ñ", "",   x1$text)
x1$text <- gsub("_\235_Õ", "",   x1$text)
x1$text <- gsub("_\220", "",   x1$text)
x1$text <- gsub("_\235_¢", "",   x1$text)
x1$text <- gsub("_\235_Ï", "",   x1$text)
x1$text <- gsub("_\235_¥", "",   x1$text)
...
# print(head(x1))
          ...
          corp <- VCorpus(DataframeSource(x1),readerControl = list(language = "en"))
             ...
             ##Clean text based on thousands of examples
            for (j in seq(corp)) {
                corp[[j]] <- gsub("\\t", " ", corp[[j]])
                ...
                corp[[j]] <- gsub("ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ€™", " ", corp[[j]])
...
# print(head(dtm$dimnames$Docs))
            ui = unique(dtm$i)
            dtm = dtm[ui,]
values$tnames <- dtm$dimnames$Docs
# print(length(values$tnames))
             print(dtm)
...
values$top_10_words<-top_10_words
			...
			# print(table(is.na(values$x1phu$dropped)))
			# if (length(table(is.na(values$x1phu$dropped)))>1){
            names(corp) <- values$tnames#values$x1phu$doc_id[is.na(values$x1phu$dropped)]#rownames(tphu) 10.2.22 PHUDCFILY
                 # print(head(names(corp),59))
				 # print(length(names(corp)))
			# }else{
           # names(corp) <- values$x1phu$doc_id#rownames(tphu) 10.2.22 PHUDCFILY
                # # print(names(corp))
# }			
        ...
        ntextlda <- eventReactive(input$goButton, {
            ...
            # corp <- VCorpus(VectorSource(dtm2list))
            # if (length(table(is.na(values$x1phu$dropped)))>1){
            # names(corp) <- values$x1phu$doc_id[is.na(values$x1phu$dropped)]#rownames(tphu) 10.2.22 PHUDCFILY
                # # print(names(corp))
			# }else{
           # names(corp) <- values$x1phu$doc_id#rownames(tphu) 10.2.22 PHUDCFILY
                # # print(names(corp))
...
trp_10_words<-data.frame(words=names(head((sort(rowSums(as.matrix(t(dtm))),decreasing=TRUE)),20)),freq=head((sort(rowSums(as.matrix(t(dtm))),decreasing=TRUE)),20), row.names=NULL)
 ...
 FindTopicsNumberPHU <- function (dtm, topics = seq(10, 40, by = 10), metrics = "Griffiths2004", 
    ...
    return_models = FALSE, verbose = FALSE, libpath = NULL) 
{
    ...
    if ("Griffiths2004" %in% metrics) {
        if (method == "VEM") {
            ...
            if (!"keep" %in% names(control)) 
                control <- c(control, keep = input$alpuser)
        }
    }
    if (verbose) 
        cat("fit models...")
    ...
    parallel::clusterExport(varlist = c("dtm", "method", 
        "control"), envir = environment())
    models <- parallel::parLapply(cl = cl, X = topics, fun = function(x) {
        if (is.null(libpath) == FALSE) {
            .libPaths(libpath)
        }
    ...
    for (m in metrics) {
        if (verbose) 
            cat(sprintf("  %s...", m))
        if (!m %in% c("Griffiths2004", "CaoJuan2009", 
            "Arun2010", "Deveaud2014")) {
            cat(" unknown!\n")
        }
        else {
            result[m] <- switch(m, Griffiths2004 = Griffiths2004(models, 
                control), CaoJuan2009 = CaoJuan2009(models), 
                Arun2010 = Arun2010(models, dtm), Deveaud2014 = Deveaud2014(models), 
                NaN)
            if (verbose) 
                cat(" done.\n")
        }
...
###changes for order match PHUDCFILY
            fitted<-ap_lda
            corpus<-values$corp
            doc_term<-values$dtm
            
            phi <- posterior(fitted)$terms %>% as.matrix
            theta <- posterior(fitted)$topics %>% as.matrix#; colnames(theta)<-NULL#names(as.data.frame(ap_lda@gamma))
            #print(theta[rownames(theta)=="7827 2019_8530", ])#New
            vocab <- colnames(phi)
            doc_length <- vector()
...
# print(head(as.data.frame(topicProbabilities))) #This is new 10/2/22 PHUDCFILY
...
# names(topicProbabilities)[2] <- "row No. ID"
